...
> `setup.sql` ใช้สำหรับฐานข้อมูลใหม่และควรรันครั้งเดียว หากพบ error ให้บันทึกข้อความ
> ที่แสดงแล้วตรวจสอบสาเหตุก่อน ไม่ควรวางไฟล์ทั้งหมดแล้วรันซ้ำทันที
...
## ขั้นตอนที่ 7 — License การใช้งาน

หลังติดตั้งเสร็จ ระบบจะให้ **ทดลองใช้งานฟรี 90 วัน** โดยอัตโนมัติ ไม่ต้องทำอะไรเพิ่ม

### สถานะ License

| แถบที่เห็น | ความหมาย | สิ่งที่ทำได้ |
|-----------|----------|-------------|
| แถบเหลืองด้านบน "ทดลองใช้งาน · เหลืออีก X วัน" | อยู่ในช่วงทดลอง | ใช้งานได้ทุกฟีเจอร์ |
| แถบแดงด้านบน "หมดอายุทดลองใช้" | ครบ 90 วันแล้ว | ดูข้อมูลได้อย่างเดียว บันทึกไม่ได้ |
| ไม่มีแถบ (badge เขียว "ลิขสิทธิ์ถาวร") | มี License แล้ว | ใช้งานได้ทุกฟีเจอร์ตลอดไป |

### เมื่อต้องการซื้อ License

1. ติดต่อผู้พัฒนา [topchanon@gmail.com](mailto:topchanon@gmail.com)
2. แจ้ง **ชื่อโรงเรียน** (ตรงกับที่กรอกในระบบ → เมนู ข้อมูลโรงเรียน)
3. ผู้พัฒนาจะส่ง License Key รูปแบบ `XXXX-XXXX-XXXX-XXXX` ให้ทางอีเมล

### วิธีกรอก License Key

1. เข้าระบบด้วย account ผู้ดูแลระบบ (admin)
2. คลิกปุ่ม **"ซื้อ License →"** ที่แถบด้านบน หรือเปิด `<URL ระบบ>/license`
3. กรอก License Key ที่ได้รับ → คลิก **"ยืนยัน License Key"**
4. ระบบจะแสดง badge **"ลิขสิทธิ์ถาวร"** ทันที

> ⚠️ License ผูกกับ **ชื่อโรงเรียน** ในระบบ — ถ้าเปลี่ยนชื่อโรงเรียนในภายหลัง
> จะต้องขอ License Key ใหม่

---

## ขั้นตอนที่ 8 — ตั้งค่าโรงเรียนครั้งแรก
...
> ⚠️ **ชื่อโรงเรียนนี้จะถูกใช้ผูกกับ License** — กรอกให้ถูกต้องตั้งแต่แรก
...
### SQL เพิ่มสำหรับระบบเชื่อมเช็กชื่อ (เวอร์ชัน 1.1.0)

โรงเรียนที่ติดตั้งก่อนเวอร์ชัน 1.1.0 หลัง Sync fork แล้ว ต้องเปิดไฟล์
`migrations/20260914_link_daily_subject_attendance.sql` ก๊อปทั้งหมด และรันหนึ่งครั้งใน
Supabase → SQL Editor เพื่อเปิดใช้การตั้งวันเรียนและดึงเช็กชื่อรายวันเข้าสู่รายวิชา

SQL ชุดนี้เพิ่มตารางใหม่เท่านั้น ไม่ลบหรือแก้ไขข้อมูลเช็กชื่อเดิม
